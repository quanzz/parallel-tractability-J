...
\item add \texttt{rch}$(A(a),B(a))$ to $S_{\textit{\!\tiny rch}}$
  ...
  A\sqsubseteq_* B$ \BG{By referring to the entailment relation, the *
  seems not needed: $\mathcal{O}\models
  A\sqsubseteq B$. Similarly below. I noticed that, when checking
  whether we introduced the * notation with a clear reference to the
  ontology/RBox, but we no not. I added a sentence to say that we omit
the RBox (as we never introduced the * w.r.t.\ the whole ontology) if
it is clear from the context. } and $A(a)\in V$;
