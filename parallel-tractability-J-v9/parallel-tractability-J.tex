...
(\uppercase\expandafter{\romannumeral1}) Given a class of datalog programs $\mathbb{P}$ where
a rule set is shared for each datalog program $P=\langle R, \textbf{I}\rangle$ in $\mathbb{P}$,
it can be checked that $|P^*|$ is polynomial in the size of $\textbf{I}$. 
This is because that each predicate occurring in $\mathbb{P}$ has a constant arity
and Algorithm~$\mathsf{A}_{bsc}$ does not introduce
new constants. This also means that the number of processors is polynomially bounded.
...
iteration of \ref{alg1:updateG}. Since each processor handles one rule instance, 
\ref{alg1:updateG} costs one time unit on each processor in theory. 
Assume that there is no delay and other communication problems among all the processors.
We have that the computing time of \ref{alg1:updateG} of the whole algorithm costs one time unit as well.
Further, the whole computing time turns out to be bounded by the number of iterations of \ref{alg1:updateG}. (\uppercase\expandafter{\romannumeral4})
