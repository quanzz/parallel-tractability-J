...
Let $\delta^*_R$ be the transitive closure of $\delta_R$ where $R$ is a transitive role.\footnote{Note that,
...
In the above definition, for a role sequence $L=R_1R_2,\ldots ,R_n$, let $L^-=R_n^-,\ldots ,R_2^-R_1^-$.
...
% \item $R(a,b)$, for each $R'(a,b)\in\mathcal{A}$ and $R'\sqsubseteq_{*}R$;
% \item $R(a,b)$, for each $R'(b,a)\in\mathcal{A}$ and $R'\sqsubseteq_{*}R^-$;
% \item $R(a,b)$, for each $R_0R_1,\ldots ,R_n\in\mathcal{L}(R)$, where $R_i(x_i,x_{i+1})\in\mathcal{A}$ and $x_0=a, x_{n+1}=b$ for $i\in\{0,1,\ldots ,n\}$.
% \end{enumerate}
\end{definition}
...
\item $R(a,b)$ is derived by applying the rule (R4), w.l.o.g.,
    ...
    \item If there exists a role sequence
      $R_0R_1,\ldots ,R_m\in\mathcal{L}(R')$ s.t.\ $R_j(x_j,x_{j+1})\in\mathcal{A}$ and $x_0=c_i, x_{n+1}=c_{i+1}$ for $i\in\{0,1,\ldots ,m\}$
        and $R'\sqsubseteq_* R_i$, then $L_i=R_0R_1,\ldots ,R_m$ holds.
    \end{enumerate}

    Based on $L_i, i\in\{1,2\}$, we have that $L_1L_2\in\mathcal{L}(R)$. Further $R(a,b)\in\delta_R$
    holds.\label{dhlplus:NTIcaseR4}

\item $R(a,b)$ is derived by applying the rule (R3). This case is impossible,
    since $R$ is a simple but not a transitive role.
\end{enumerate}
