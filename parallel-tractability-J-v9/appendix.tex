...
% \item $R(a,b)$, for each $R'(a,b)\in\mathcal{A}$ and $R'\sqsubseteq_{*}R$;
% \item $R(a,b)$, for each $R'(b,a)\in\mathcal{A}$ and $R'\sqsubseteq_{*}R^-$;
% \item $R(a,b)$, for each $R_0R_1,...,R_n\in\mathcal{L}(R)$, where $R_i(x_i,x_{i+1})\in\mathcal{A}$ and $x_0=a, x_{n+1}=b$ for $i\in\{0,1,...,n\}$.
% \end{enumerate}
\end{definition}
...
\item $R(a,b)$ is derived by applying the rule (R4), w.l.o.g.,
    ...
    \item If there exists a role sequence
      $R_0R_1,...,R_m\in\mathcal{L}(R')$ s.t.\ $R_j(x_j,x_{j+1})\in\mathcal{A}$ and $x_0=c_i, x_{n+1}=c_{i+1}$ for $i\in\{0,1,...,m\}$
        and $R'\sqsubseteq_* R_i$, then $L_i=R_0R_1,...,R_m$ holds.
    \end{enumerate}
