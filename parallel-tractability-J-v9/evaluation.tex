...
\caption{The statistics of the test ontologies, where $\sharp$concept
  ...
  the number of assertions in the ontology\BG{I swapped the column
    order to have TBox and ABox statistics together rather than
    signature size and then number of axioms and assertions,
    analogously for Table~\ref{tab:generated}}}
...
reciprocals \BG{Why the reciprocals and not directly the number of threads?} of the numbers of threads and
...
\caption{(left) The reasoning times over the LUBM ontologies; (right)
  the reasoning times over yago-core \BG{Could you swap the legend
    entries for (left) so that ParallelDHL is given above RDFox as in (right)}}
...
\caption{The numbers of nodes in different depths of lubm-250 (left)
  and yago-core (right) \BG{Could you move the right diagram away from
  the border so that the axis marks do not run into the border?}}
...
describe the reference relationships among articles:\BG{I assume you
  mean \texttt{CollegeArtic*le*} instead of the consistently used
  \texttt{CollegeArtic*al*}. Global search and replace should fix that...}
\begin{align}
...
\caption{(left) reasoning times over the NPTU datasets; (right)
  reasoning times over the PTU datasets\BG{Could we consistently have
    first ParellelDHL and then RDFox in the legends (also in the text
    I prefer to name ParallelDHL first as it is the main system
    treated in the article). I would also suggest to first (left) have
  the PTU and then (right) the NPTU diagram as this is the order now
  consistently used in the text and PTU is the main focus here. }}
...
\BG{left should be adjusted to right if you follow my suggestion to
  swap the two diagrams; analogously for the next sentence}.
...
considering that the input ontology sizes are close.\BG{For this whole
paragraph (and the figures) I would prefer to first discuss the PT
case (PTU) and then the non PT (NPTU) case and always list/name the main
system (ParallelDHL) first and then RDFox. }
